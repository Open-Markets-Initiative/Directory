...
Fix 4.2 order entry protocol for submitting, replacing, and cancelling orders on the CIX ASPEN order book of the CIX ATS, with fills and order status returned via Execution Reports. The single CIX order entry gateway routes to this book via ExDestination(100)=ASPC.
...
- **Book routing** - Destination book selected per order via ExDestination(100)=ASPC
