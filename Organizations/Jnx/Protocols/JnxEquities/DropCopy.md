## JnxEquities Drop Copy: Japannext Fix Drop Copy Specification Equities

Financial Information eXchange encoding of the Japannext drop copy service for equities, a listen only Fix session that mirrors the execution reports of the trading session so a participant can reconcile its order state without consuming the order entry connection.
...
- **Cross protocol** - Orders entered over Ouch are reported here too
