## Lse Level2Mbo Replay: Level 2 MBO Replay

Replay service for the London Stock Exchange Level 2 MBO market data gateway, carrying individual displayed orders over a Tcp session using Group Ticker Plant framing.

### Overview

Level 2 MBO Replay is the retransmission service provided by the Lseg Group Ticker Plant Level 2 MBO market data gateway for London Stock Exchange. Each gateway includes its own replay service caching the most recent application messages published on that gateway's real time channel, so a replay session returns individual displayed orders rather than the full cross service line message library.
