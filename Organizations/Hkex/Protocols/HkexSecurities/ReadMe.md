## Hkex Securities Market


The Hkex Securities Market is operated by The Stock Exchange of Hong Kong Limited (SEHK), a wholly owned subsidiary of Hong Kong Exchanges and Clearing (Hkex). It covers all cash-market listed securities including ordinary shares, preference shares, rights, depository receipts, SPAC shares, derivative warrants, CBBCs, inline warrants, equity warrants, ETFs, REITs, unit trusts, leveraged and inverse products, and debt securities. Market data is disseminated via the Orion Market Data - Cash (OMD-C) platform.


### Protocols


| Protocol | Type | Note |
| --- | --- | --- |
| [Combined](Combined.md) | MarketData | Orion Market Data Cash Combined |
| [CombinedRefresh](CombinedRefresh.md) | MarketData | Orion Market Data Cash Combined Refresh |
| [CombinedRetrans](CombinedRetrans.md) | MarketData | Orion Market Data Cash Combined Retransmission |
| [FullTick](FullTick.md) | MarketData | Orion Market Data Cash FullTick |
| [FullTickRefresh](FullTickRefresh.md) | MarketData | Orion Market Data Cash FullTick Refresh |
| [FullTickRetrans](FullTickRetrans.md) | MarketData | Orion Market Data Cash FullTick Retransmission |
| [Index](Index.md) | MarketData | Orion Market Data Cash Index |
| [IndexRefresh](IndexRefresh.md) | MarketData | Orion Market Data Cash Index Refresh |
| [IndexRetrans](IndexRetrans.md) | MarketData | Orion Market Data Cash Index Retransmission |
| [Mmdh](Mmdh.md) | MarketData | Orion Market Data Cash Mainland Market Data Hub |
| [Premium](Premium.md) | MarketData | Orion Market Data Cash Premium |
| [PremiumRefresh](PremiumRefresh.md) | MarketData | Orion Market Data Cash Premium Refresh |
| [PremiumRetrans](PremiumRetrans.md) | MarketData | Orion Market Data Cash Premium Retransmission |
| [Standard](Standard.md) | MarketData | Orion Market Data Cash Standard |
| [StandardRefresh](StandardRefresh.md) | MarketData | Orion Market Data Cash Standard Refresh |
| [StandardRetrans](StandardRetrans.md) | MarketData | Orion Market Data Cash Standard Retransmission |

## Specifications

Protocol documentation is available in the [Specifications](../../Specifications) directory.

