## MoonAts Retransmission: TCP binary retransmission service for MOON ATS overnight trading recovery

TCP binary retransmission service supporting recovery for the OTC extended-hours market data feeds (Depth of Book and Top of Book). Session-based flow: Login, Retransmission Request/Response, Spin Request/Response, Enhanced Spin Request/Response, Heartbeat. Delivers gap fills for sequenced UDP messages that a subscriber missed, plus spin/enhanced-spin snapshots for mid-session initialization. Used by MOON ATS (Markets Open Overnight) for extended overnight trading of NMS-listed US equity securities.
...
- **Channel pairing** - Companion to the MOON ATS Depth of Book and Top of Book multicast channels
