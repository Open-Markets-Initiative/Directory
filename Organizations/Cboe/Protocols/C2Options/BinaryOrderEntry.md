## C2Options Binary Order Entry: Cboe C2 Options Binary Order Entry

Cboe Binary Order Entry (Boe) protocol for submitting, modifying, and cancelling orders on Cboe C2 Options Exchange.

### Overview

Order Entry is the Cboe Binary Order Entry (Boe) protocol for Cboe C2 Options Exchange, providing members with a low-latency binary interface to submit, modify, and cancel orders. The wire format uses compact fixed-width binary messages and a custom session layer with authentication, sequence tracking, and heartbeat monitoring.
