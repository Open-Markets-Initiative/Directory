## BzxEquities Multicast Depth Of Book: Cboe BZX Full Depth Of Book Data

Full depth of book Pitch feed publishing order add, modify, cancel, and execute events for equities traded on Cboe US Equities BZX Exchange.

### Overview

Depth Of Book is the full depth of book market data feed for Cboe US Equities BZX Exchange, delivered in the Cboe Pitch binary message format. The feed publishes order-by-order events including add, modify, delete, and execute messages so that subscribers can reconstruct the complete order book for every listed equity instrument.
