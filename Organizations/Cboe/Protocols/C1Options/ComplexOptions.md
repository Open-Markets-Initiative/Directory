## C1Options Complex Options: Cboe Options Complex Order Entry

Cboe Binary Order Entry protocol for submitting, modifying, and cancelling complex multi-leg orders on Cboe Options Exchange (C1).

### Overview

Complex Options is the Cboe Binary Order Entry variant for complex multi-leg strategies on Cboe Options Exchange (C1). It extends the standard Boe protocol with message types for defining and submitting spreads, straddles, combinations, and other multi-leg strategies, along with the order lifecycle messages needed to manage them.
