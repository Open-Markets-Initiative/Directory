## NsmEquities Orders: Nasdaq Order Entry

Ouch-based binary order entry protocol for submitting, replacing, and cancelling orders on Nasdaq Stock Market.

### Overview

Orders is the Nasdaq Ouch order entry protocol variant for Nasdaq Stock Market, providing members with a low-latency binary interface to submit, replace, and cancel orders for equities. The wire format is the Ouch message family with fixed-width binary fields and numeric token references for efficient order management.
