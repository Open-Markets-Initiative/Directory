## LtseEquities Memoir Last Sale: Ltse Last Sale Trade Data

Real-time trade feed reporting executions, cancellations, and corrections for securities traded on the Long-Term Stock Exchange.

### Overview

Memoir Last Sale is an event-based market data protocol that publishes trade activity for every instrument traded on the Long-Term Stock Exchange. The feed delivers trade reporting, trade cancellation, and trade correction events along with trading session state, enabling subscribers to consume a clean last-sale stream without needing full depth of book.
